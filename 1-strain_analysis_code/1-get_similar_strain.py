...
'''
python get_similar_strain.py --input input_file_loc --output1  output1 --output2 output2 --output3 output3 --cutoff n
'''
...
parser.add_argument('--output2', help='all the comparison result with cutoff',
                    default=True)
parser.add_argument('--output3', help='all the comparison result with cutoff',
                    default=True)
...
# input_file_list = '/media/saidi/Elements/Project/Project17_mixtureS_from_Xin_orginal/latest/MixtureS/Ming/two_codes_v1/two_codes/similar_mutations_between_strains/input_file_loc'
# output_file1 = '/media/saidi/Elements/Project/Project17_mixtureS_from_Xin_orginal/latest/MixtureS/Ming/two_codes_v1/two_codes/similar_mutations_between_strains/test_result/all_result'
# output_file2 = '/media/saidi/Elements/Project/Project17_mixtureS_from_Xin_orginal/latest/MixtureS/Ming/two_codes_v1/two_codes/similar_mutations_between_strains/test_result/all_result_cutoff'
# output_file3 = '/media/saidi/Elements/Project/Project17_mixtureS_from_Xin_orginal/latest/MixtureS/Ming/two_codes_v1/two_codes/similar_mutations_between_strains/test_result/all_result_cutoff_both'
# cutoff = 0.9
...
def getCommonPer(dict1,dict2):
    ...
    for key1,val1 in dict1.items():
        for key2,val2 in dict2.items():
            ...
            per1 = len(common_list)/len(val1)
            per2 = len(common_list)/len(val2)
            common_num = len(common_list)
            dict3[key1 + '--' + key2] = [per1,per2,common_num,common_list]
...
def getOutput1(input_file_list,output_file1):
    ...
    for strain1 in all_strain_name:
        ...
        for strain2 in all_strain_name:
            per = dict_all[strain1 + '--' + strain2][0]
...
############ get the similar strains with cutoff(both meet the criteria) ##################
def getOutput3(input_file_list, output_file2,cutoff):
    dict_all = getDict_all(input_file_list)
    ...
    for strain1 in all_strain_name:
        ...
        for strain2 in all_strain_name:
            if strain1 != strain2:
                ...
                per1 = per[0]
                per2 = per[1]
                if per1 >= cutoff and per2 >= cutoff:
                    temp_per_list.append(strain2 + '\t' + '\t'.join([str(i) for i in per]))
        temp_per_list = sorted(temp_per_list,key=lambda x:float(x.split('\t')[1]),reverse=True)
        f2.write('>' + strain1 + '\n' + '\n'.join(temp_per_list) + '\n')
...
getOutput3(input_file_list,output_file3,cutoff)
